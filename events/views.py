...
def event_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('registration_success')  # Redirect to a home page or dashboard
    else:
...
def registered_users(request):
    ...
    return render(request, 'events/registered_users.html', {'users': users})
...
def export_events_to_word(request):
    from django.http import HttpResponse
    from docx import Document
    from django.utils.html import strip_tags
    ...
    headers[3].text = 'End Date'
    
    # Add event data
    events = Event.objects.all()
    for event in events:
        row_cells = table.add_row().cells
        row_cells[0].text = event.name
        row_cells[1].text = str(event.category)
        row_cells[2].text = event.start_date.strftime('%Y-%m-%d') if event.start_date else 'N/A'
        row_cells[3].text = event.end_date.strftime('%Y-%m-%d') if event.end_date else 'N/A'
    
    # Create the HttpResponse object with the appropriate headers
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
    response['Content-Disposition'] = 'attachment; filename=events_export.docx'
    doc.save(response)
...
from django.http import HttpResponse
# from docx import Document
from .models import Event  # Assuming your model for events is named `Event`

def download_event_detail(request, event_id):
    try:
        ...
        # Add event details
        document.add_paragraph(f"Event Name: {event.name}")
        document.add_paragraph(f"Category: {event.category}")
        document.add_paragraph(f"Description: {strip_tags(event.description) if event.description else 'N/A'}")
        document.add_paragraph(f"Venue: {event.venue or 'N/A'}")
        document.add_paragraph(f"Location: {event.location or 'N/A'}")
        document.add_paragraph(f"Start Date: {event.start_date.strftime('%Y-%m-%d %H:%M') if event.start_date else 'N/A'}")
        document.add_paragraph(f"End Date: {event.end_date.strftime('%Y-%m-%d %H:%M') if event.end_date else 'N/A'}")
        document.add_paragraph(f"Status: {event.status}")
        document.add_paragraph(f"Scheduled Status: {event.scheduled_status}")
        document.add_paragraph(f"Points: {event.points or 'N/A'}")
        document.add_paragraph(f"Maximum Attendees: {event.maximum_attende or 'N/A'}")
        
        # Create the HTTP response
        from django.http import HttpResponse
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
        response['Content-Disposition'] = f'attachment; filename=event_{event.id}_details.docx'
